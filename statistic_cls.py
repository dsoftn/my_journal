...
class Statistic(QDialog):
    ...
    def lbl_block_info_kw_link_hovered(self, link):
        ...
        block_id = UTILS.TextUtility.get_integer(link[2:])
        if block_id:
            db_block = db_record_cls.Record(self._stt, block_id)
    ...
    def lbl_def_info_kw_link_hovered(self, link):
        ...
        def_id = UTILS.TextUtility.get_integer(link[2:])
        if def_id:
            db_def = db_definition_cls.Definition(self._stt, def_id)
    ...
    def _populate_block_info_label(self):
        ...
        bpd_total_entry_days_percent = UTILS.TextUtility.number_to_string_formatted((bpd_total_entry_days / bpd_elapsed_total.total_days) * 100, decimals=2)
        ...
        bpd_blocks_per_day = UTILS.TextUtility.number_to_string_formatted(block_count / bpd_total_entry_days, decimals=2)
        sentences_per_day = UTILS.TextUtility.number_to_string_formatted(total_sentences / bpd_total_entry_days, decimals=2)
        words_per_day = UTILS.TextUtility.number_to_string_formatted(total_words / bpd_total_entry_days, decimals=2)
        characters_per_day = UTILS.TextUtility.number_to_string_formatted(total_characters / bpd_total_entry_days, decimals=2)
        ...
        blocks_with_tags_percent = UTILS.TextUtility.number_to_string_formatted((len(blocks_with_tags) / block_count) * 100, decimals=2)
        tags_avg_all = UTILS.TextUtility.number_to_string_formatted(total_tags / block_count, decimals=2)
        tags_avg = UTILS.TextUtility.number_to_string_formatted(total_tags / len(blocks_with_tags), decimals=2)
        blocks_with_media_percent = UTILS.TextUtility.number_to_string_formatted((len(blocks_with_media) / block_count) * 100, decimals=2)
        media_avg_all = UTILS.TextUtility.number_to_string_formatted(total_media / block_count, decimals=2)
        media_avg = UTILS.TextUtility.number_to_string_formatted(total_media / len(blocks_with_media), decimals=2)
    ...
    def _populate_def_info_label(self):
        ...
        self.update_info_label(self.lbl_def_info, self.getl("statistic_def_syn"), [UTILS.TextUtility.number_to_string_formatted(defs_without_syn), UTILS.TextUtility.number_to_string_formatted((defs_without_syn / total_defs) * 100, decimals=2)])
        ...
        self.update_info_label(self.lbl_def_info, self.getl("statistic_def_syn_repeated"), [UTILS.TextUtility.number_to_string_formatted(repeated_syn), UTILS.TextUtility.number_to_string_formatted((repeated_syn / total_syn) * 100, decimals=2)])
        ...
        self.update_info_label(self.lbl_def_info, self.getl("statistic_def_desc_avg"), [UTILS.TextUtility.number_to_string_formatted(total_desc_sentences / total_defs), UTILS.TextUtility.number_to_string_formatted(total_desc_words / total_defs), UTILS.TextUtility.number_to_string_formatted(avg_desc_chars)])
        ...
        self.update_info_label(self.lbl_def_info, self.getl("statistic_def_img"), [UTILS.TextUtility.number_to_string_formatted(defs_without_img), UTILS.TextUtility.number_to_string_formatted((defs_without_img / total_defs) * 100, decimals=2)])
        ...
        self.update_info_label(self.lbl_def_info, self.getl("statistic_def_img_repeated"), [UTILS.TextUtility.number_to_string_formatted(repeated_img), UTILS.TextUtility.number_to_string_formatted((repeated_img / total_img) * 100, decimals=2)])

        # Default image in definitions
        self.update_info_label(self.lbl_def_info, "\n" + self.getl("statistic_def_default_img_title"), fg_color="#ffff00")

        if defs_without_default_image:
            self.update_info_label(self.lbl_def_info, self.getl("statistic_def_default_img"), [UTILS.TextUtility.number_to_string_formatted(len(defs_without_default_image)), UTILS.TextUtility.number_to_string_formatted((len(defs_without_default_image) / total_defs) * 100, decimals=2)])
