...
class NewsMondo(AbstractNewsSource):
    ...
    def _get_page_tags(self) -> list:
        self.rashomon.clear_errors()
        html_code = self.rashomon.get_segment_selection("tags_000")
        if self.rashomon.errors() or not html_code:
            return None
        
        page_tags = []
        self.html_parser.load_html_code(html_code)
        tag_slices = self.html_parser.get_all_text_slices()
        if tag_slices:
            for txt_slice in tag_slices:
                text = txt_slice.txt_value
                link = txt_slice.txt_link
                if link:
    ...
    def load_categories(self) -> bool:
        ...
        for main_cat in lists[0].sub_items:
            ...
            result[str(main_count)]["url"] = main_cat.get_link_url()
            ...
            for sub_cat in main_cat.sub_items:
                ...
                result[str(main_count)]["sub"][str(sub_count)]["url"] = sub_cat.get_link_url()
                ...
                if sub_cat.sub_items:
                    for extra_sub_cat in sub_cat.sub_items:
                        ...
                        result[str(main_count)]["sub"][str(sub_count)]["url"] = extra_sub_cat.get_link_url()
                        result[str(main_count)]["sub"][str(sub_count)]["sub"] = {}
