...
class AutoComplete(QFrame):
    ...
    def _find_last_word(self, txt: str, number_of_words: int = None) -> str:
        ...
        # Find begining of sentence and work only with that part of text
        begin_of_sen = 0
        txt = ". " + txt
        for i in self.END_OF_SENTENCE:
...
class TextHandler():
    ...
    def check_definitions(self, e: QtGui.QMouseEvent = None):
        ...
        for i in self._def_list:
            ...
            while pos >= 0:
                def_s = i[0]
                for j in self.END_OF_SENTENCE:
                    if i[0].find(j) >= 0:
                        def_s = def_s.strip()
                        pos = txt1.find(def_s, pos)
                        break
                else:
                    pos = txt.find(def_s, pos)
                if pos >=0:
                    if pos == 0:
                        self._text_def_map.append([i[1], i[0], pos, pos + len(i[0]) - 1])
                    else:
    ...
    def definition_hint(self, duration: int, record_id: int):
        ...
        self._definition_hint.handle_text_event(self.txt_box.toPlainText(), pos, self._text_def_map, duration, record_id=record_id)
...
class DefinitonHint(QFrame):
    ...
    def handle_text_event(self, txt: str, position: QPoint, def_map: list, duration: int, record_id: int = None) -> bool:
        ...
        last_word = self._get_last_word(txt, def_map)
        if not last_word:
            return False
    ...
    def _get_last_word(self, txt: str, def_map: list) -> str:
        if not txt.strip():
            return None
