...
class SynonymManager(QFrame):
    ...
    def _get_section(self, html: str) -> list:
        section_bounding_lines = None

        max_lines = len(html.split("\n"))

        h2_codes = self.html_parser.get_tags(html_code=html, tag="h2", return_line_numbers=True)
        
        for idx, code in enumerate(h2_codes):
            span_code = self.html_parser.get_tags(html_code=code[0], tag="span", custom_tag_property=[["class", "mw-headline"], ["id", self.section]])
            if span_code:
                if idx < len(h2_codes) - 1:
                    max_lines = h2_codes[idx + 1][1]
                section_bounding_lines = [code[1], max_lines]
                break
        result_code = ""
        for idx, line in enumerate(html.split("\n")):
            if section_bounding_lines:
                if idx >= section_bounding_lines[0] and idx < section_bounding_lines[1]:
                    result_code += line + "\n"
